...
The default provider is empty. The Hypermedea adapter installs its
`JasonInteractionLog` as the provider when its registry is loaded.
...
In this repository, the service file currently lives at:

```text
src/main/resources/META-INF/services/ccrs.core.contingency.CcrsStrategyProvider
```

If the project is split into modules later, each optional capability module
should carry its own service file under its own `src/main/resources`. For
example, `ccrs-langchain4j` would list only
`Langchain4jPredictionStrategyProvider`, and `ccrs-a2a` would list only
