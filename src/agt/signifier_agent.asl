...
+!track_unexplored_transitions(CurrentCell) :
    ccrs(O)[source(CurrentCell), ccrs_type("Signifier")] // If CurrentCell has an Object annotaed as CCRS Signifier
    <-
        if (not remaining(CurrentCell, _)) { // Ensure this is only added once to keep track of explored paths
            ...
            .findall(Y, (transition(_, Y)[valid_transition("True")] & ccrs(Y)[ccrs_type("Signifier")]), SigList) ; // Union of currently valid transition Options and CCRS Signifier Objects
            .print("************ CCRS ****************   ", SigList) ;
